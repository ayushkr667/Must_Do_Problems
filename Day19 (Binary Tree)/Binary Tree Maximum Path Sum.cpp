...
public:
    int solve(TreeNode* node, int& res){
        ...
        int left_sum = solve(node->left, res);
        int right_sum = solve(node->right, res);
        
        int temp = max ( (max(left_sum, right_sum) + node ->val), node->val);
        temp = max(temp,  left_sum + right_sum + node->val); 
        res = max(res, temp);
